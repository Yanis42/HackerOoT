...
s32 Camera_Parallel1(Camera* camera) {
    ...
    *eyeNext = Camera_AddVecGeoToVec3f(at, &spA8);
    ...
    //! @bug the original code doesn't return
    return 1;
}
...
s32 Camera_UpdateWater(Camera* camera) {
    ...
    //! @bug: the original code doesn't always return a value, but sometimes does.
    return 1;
}
...
if ((Camera_RequestSettingImpl(camera, requestedCamSetting,
    ...
    //! @note: the original code doesn't return here, but return is unused
    return 1;
}
...
s32 Camera_SetCSParams(Camera * camera, CutsceneCameraPoint * atPoints, CutsceneCameraPoint * eyePoints,
                        Player * player, s16 relativeToPlayer) {
    PosRot playerPosRot;
