...
fix dyn table size determination, or just require a table spec to be provided

some dyn tables are aligned to 2 bytes so a single UNK preceding it may be due to padding

unused stuff between layers and channels can be assumed to be more layers (?)

unknowns between dyn tables and envelopes are envelopes (?)

unknowns between envelopes are envelopes (?)

unknowns between envelopes and the end of the file are envelopes (?)
	EXCEPT when the end of the file is zeros then its more likely to be filters

sequence beginning with testchan 0 is a buffer (?)
OR any ldseq is an array and an array of 0 is a buffer (?)

when channel and layer overlap emit channel and add bug marker for missing layer end
"""
